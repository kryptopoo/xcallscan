...
export class CelatoneScan implements IScan {
    ...
    async getEventLogs(flagNumber: number, eventName: string): Promise<{ lastFlagNumber: number; eventLogs: EventLog[] }> {
        ...
        const flagPageIndex = totalPages - Math.ceil(flagNumber / limit)
        const txsRes = await this.callApi(`${API_URL[this.network]}/accounts/${CONTRACT[this.network].xcall}/txs`, {
            limit: limit,
            offset: (totalPages - flagPageIndex) * limit,
            is_wasm: true,
            is_execute: true
        })
        ...
        for (let i = 0; i < txs.length; i++) {
            ...
            for (let index = 0; index < eventNames.length; index++) {
                ...
                if (decodeEventLog) {
                    let log: EventLog = {
                        txRaw: txRes.tx_response.raw_log,
                        blockNumber: Number(txRes.tx_response.height),
                        blockTimestamp: Math.floor(new Date(txRes.tx_response.timestamp).getTime() / 1000),
                        txHash: txRes.tx_response.txhash,
                        txFrom: txRes.tx_response.tx.body.messages[0].sender,
                        txTo: txRes.tx_response.tx.body.messages[0].contract ?? '',
                        txFee: txRes.tx_response.tx.auth_info.fee.amount[0].amount,
                        txValue:
                            txRes.tx_response.tx.body.messages[0].msg?.cross_transfer?.amount ||
                            txRes.tx_response.tx.body.messages[0].msg?.deposit?.amount ||
                            '0',
        ...
        scanCount = scanCount >= this.totalCount ? this.totalCount : scanCount + txs.length
